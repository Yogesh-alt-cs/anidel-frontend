...
const InfoLayout = ({ data, showBigPoster }) => {
  ...
  return (
    <>
      <div className="banner min-h-[700px] relative w-full bg-card pt-10 md:pt-20">
          ...
          <div className="right mt-3 w-full flex flex-col gap-2">
            <div className="path hidden md:flex items-center gap-2 text-base ">
              ...
              <Link to={`/animes/${data.type.toLowerCase()}`}>
                <h4 className="hover:text-primary-hover">{data.type}</h4>
              </Link>
              <span className="h-1 w-1  rounded-full bg-primary"></span>
            ...
            <div className="sounds flex items-center gap-2 my-2">
              ...
              />
              <span className="h-1 w-1 rounded-full bg-primary"></span>
              <span className="type text-text-secondary text-sm font-bold">
                {data.type}
              </span>
              <span className="h-1 w-1 rounded-full bg-primary"></span>
              <span className="duration text-text-secondary text-sm font-bold">
                {data.duration}
              </span>
            </div>
            ...
            {data.synopsis && (
              <div className="overview">
                <p
                  className={`${showFull ? "line-clamp-none" : "line-clamp-3"
                    } text-balance text-text-secondary`}
                >
                  {data.synopsis}
                </p>
                <span
