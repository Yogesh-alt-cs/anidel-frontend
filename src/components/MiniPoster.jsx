...
const MiniPoster = ({ item }) => {
  return (
    <motion.div
      key={item.id}
      className="flex border-b border-white/5 last:border-b-0 pb-3 items-center gap-4 group hover:bg-[var(--surface-level-1)] p-2 rounded-xl transition-all duration-300"
      whileHover={{ x: 4 }}
    ...
    >
      ...
      <div className="text flex-1">
        <Link to={`/anime/${item.id}`}>
          <h2 className="title hover:text-primary-hover mb-2 font-semibold line-clamp-2 transition-colors">
            {item.title}
          </h2>
        </Link>
        <div className="item flex items-center">
          ...
          {item.type && (
            <>
              ...
              <h2 className="text-xs text-text-muted">{item.type}</h2>
            </>
          )}
        </div>
      </div>
